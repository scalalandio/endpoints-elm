...
object HttpEmit {
  ...
  def moduleDefinition(module: ElmHttpModule)
  ...
  def endpointDefinition(endpoint: ElmEndpoint)
                        (implicit ctx: Context): Seq[String] = {

    val (qpArgs, qpTpes) = endpoint.request.url.queryParams
      .map { case (arg, tpe) => arg -> TypeEmit.typeReference(tpe, topLevel = false) }
      .unzip
    ...
    val withQueryParamsModifier = if (endpoint.request.url.queryParams.isEmpty) {
      ...
      val elmQueryParamsList = endpoint.request.url.queryParams.map { case (qpArgName, qpTpe) =>
  ...
  def urlStringLiteral(urlPrefix: String)
                      (elmUrl: ElmUrl): String = {

    val urlExpr = elmUrl.segments.collect {
  ...
  def toQueryParamsListElems(queryParamName: String,
                             elmType: ElmType): String = elmType match {
    case AppliedType.Maybe(tpe) =>
      s"""($queryParamName |> Maybe.map (\\p -> ("$queryParamName", ${toStringFunctionName(tpe).getOrElse("")} p)) |> Maybe.Extra.toList)"""
