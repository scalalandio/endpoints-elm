...
object SegmentsTest extends CodegenTest {
  ...
  trait TestEndpoints extends algebra.Endpoints with algebra.JsonSchemaEntities with algebra.JsonSchemas {
    ...
    def stringEndpoint: Endpoint[String, Unit] =
      endpoint(get(path / "string" / segment[String]("string")), emptyResponse(), tags = List("Segments"))

    def intEndpoint: Endpoint[Int, Unit] =
      endpoint(get(path / "int" / segment[Int]("int")), emptyResponse(), tags = List("Segments"))

    def longEndpoint: Endpoint[Long, Unit] =
      endpoint(get(path / "long" / segment[Long]("long")), emptyResponse(), tags = List("Segments"))

    def uuidEndpoint: Endpoint[UUID, Unit] =
      endpoint(get(path / "uuid" / segment[UUID]("uuid")), emptyResponse(), tags = List("Segments"))
  ...
  object TestElmEndpoints extends TestEndpoints with ElmCodeGenerator {
    ...
    val allEndpoints: Seq[ElmEndpoint] = Seq(stringEndpoint, intEndpoint, longEndpoint, uuidEndpoint, dateEndpoint)
  }
