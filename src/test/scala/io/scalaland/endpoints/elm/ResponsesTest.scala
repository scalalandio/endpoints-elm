...
object ResponsesTest extends CodegenTest {
  ...
  trait TestEndpoints
    ...
    def dateEndpoint: Endpoint[Unit, LocalDate] =
      endpoint(
        get(path / "json-date"),
        jsonResponse[LocalDate](),
        tags = List("Responses")
      )

    def caseClassEndpoint: Endpoint[Unit, Foo] =
      endpoint(
        get(path / "json-case-class"),
        jsonResponse[Foo](),
        tags = List("Responses")
      )
    ...
    def coproductEndpoint: Endpoint[Unit, Coproduct] =
      endpoint(
        get(path / "json-coproduct"),
        jsonResponse[Coproduct](),
        tags = List("Responses")
      )
